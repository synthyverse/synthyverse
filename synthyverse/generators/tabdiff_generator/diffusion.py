...
class UnifiedCtimeDiffusion(torch.nn.Module):
    def __init__(
        ...
        self.offsets_with_mask = torch.from_numpy(offsets[:-1]).long().to(device)
        self.mask_offsets = self.offsets_with_mask + self.mask_index
    ...
    def q_xt(self, x, move_chance, strategy='hard'):
        ...
        if strategy == 'hard':
            ...
            * x.shape, device=x.device) < move_chance
            ...
            bs = x.shape[0]
            xt_soft = torch.zeros(bs, torch.sum(self.mask_index + 1), dtype=move_chance.dtype, device=x.device)
            probs = torch.stack((1 - move_chance, move_chance), dim=-1)
            soft_sample = F.gumbel_softmax(probs.log(), tau=0.01, hard=True)
            idx = torch.stack((x + self.offsets_with_mask, self.mask_offsets.expand_as(x)), dim=-1)
            xt_soft.scatter_(1, idx.reshape(bs, -1), soft_sample.reshape(bs, -1))
            xt = torch.where(soft_sample[..., 1] > soft_sample[..., 0], self.mask_index, x)
            return xt, xt_soft
