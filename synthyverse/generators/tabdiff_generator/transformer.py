...
class Tokenizer(nn.Module):
    ...
    def __init__(self, d_numerical, categories, d_token, bias):
        ...
        if categories is None:
            ...
            self.register_buffer('category_ends', torch.tensor(categories).cumsum(0))
    ...
    def forward(self, x_num, x_cat):
        ...
        if x_cat is not None and self.category_offsets.numel():
            cat_tokens = torch.stack(
                [
                    x_cat[:, start:end] @ self.cat_weight[start:end]
