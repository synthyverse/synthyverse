...
from synthcity.plugins.core.models.tabular_ddpm.gaussian_multinomial_diffsuion import (
...
class TabDDPM(nn.Module, ValidationMixin):
    ...
    def fit(
        ...
        cat_cols = [
            (col, cnt)
            for col, vals in X[discrete_columns].items()
        ...
        dataset = TensorDataset(
            ...
            (
                torch.tensor(
                    [torch.nan] * len(X), dtype=torch.float32, device=self.device
                )
                if cond is None
                else torch.tensor(
                    cond.values,
