...
import pandas as pd
from ForestDiffusion.utils.utils_diffusion import (
    build_data_xt,
    euler_solve,
    IterForDMatrix,
    get_xt,
...
class ForestDiffusionModel:
    def __init__(
        ...
        seed=666,
        **xgboost_kwargs
    ):  # you can pass extra parameter for xgboost
        ...
        )  # since we round those, we do not need to dummy-code the binary variables
        ...
        if len(self.cat_indexes) > 0:
            X, self.X_names_before, self.X_names_after = self.dummify(
                X
        ...
        if model == "random_forest" and np.sum(np.isnan(X1)) > 0:
            raise Exception(
                "The dataset must not contain missing data in order to use model=random_forest"
            )
        ...
        if self.label_y is not None:
            assert (
                np.sum(np.isnan(self.label_y)) == 0
            ...
            for i in range(len(self.y_uniques)):
                ...
                if self.n_batch == 0:
                    self.mask_y[self.y_uniques[i]] = np.tile(
                        self.mask_y[self.y_uniques[i]], (duplicate_K)
                    )
        ...
        if self.n_batch > 0:  # Data iterator, no need to duplicate, not make xt yet
            ...
            batches = [rows_per_batch for i in range(self.n_batch - 1)] + [
                self.b - rows_per_batch * (self.n_batch - 1)
            ]
            X1_splitted = {}
            X_covs_splitted = {}
            for i in self.y_uniques:
                ...
                if X_covs is not None:
                    X_covs_splitted[i] = np.split(
                        X_covs[self.mask_y[i], :], batches, axis=0
                    )
                else:
        ...
        if self.p_in_one:
            if self.n_jobs == 1:
                ...
                for i in tqdm(range(n_steps), desc="Noise level"):
                    for j in range(len(self.y_uniques)):
                        if (
                            self.n_batch > 0
                        ):  # Data iterator, no need to duplicate, not make xt yet
                            self.regr[j][i] = self.train_iterator(
                                X1_splitted[j],
                                X_covs_splitted[j],
                                t=t_levels[i],
                                dim=None,
                            ...
                            self.regr[j][i] = self.train_parallel(
                                X_train.reshape(
                                    self.n_t, self.b * self.duplicate_K, self.c_all
                                )[i][self.mask_y[j], :],
                ...
                if (
                    ...
                    self.regr = Parallel(n_jobs=self.n_jobs)(
                        delayed(self.train_iterator)(
                            X1_splitted[j], X_covs_splitted[j], t=t_levels[i], dim=None
                        )
                        for i in tqdm(range(n_steps), desc="Noise level")
            ...
            if self.n_jobs == 1:
                self.regr = [
                    [[None for k in range(self.c)] for i in range(n_steps)]
                    for j in self.y_uniques
                ]
                for i in tqdm(range(n_steps), desc="Noise level"):
                    for j in range(len(self.y_uniques)):
                        for k in range(self.c):
                            if (
                                ...
                                self.regr[j][i][k] = self.train_iterator(
                                    X1_splitted[j],
                                    X_covs_splitted[j],
                                    t=t_levels[i],
                                    dim=k,
                ...
                if (
                    ...
                    self.regr = Parallel(n_jobs=self.n_jobs)(
                        delayed(self.train_iterator)(
                            X1_splitted[j], X_covs_splitted[j], t=t_levels[i], dim=k
                        )
                        for i in tqdm(range(n_steps), desc="Noise level")
                        for j in self.y_uniques
                        for k in range(self.c)
    ...
    def train_parallel(self, X_train, y_train):
        ...
        if self.model == "random_forest":
            ...
            )  # consider t as a golden feature if t is a variable
        elif self.model == "xgboost":
            out = xgb.XGBRegressor(
                n_estimators=self.n_estimators,
                objective="reg:squarederror",
                eta=self.eta,
                max_depth=self.max_depth,
    ...
    def my_model(self, t, y, mask_y=None, dmat=False, unflatten=True, X_covs=None):
        ...
        for j, label in enumerate(self.y_uniques):
            ...
            if mask_y[label].sum() > 0:
                if self.p_in_one:
                    out[mask_y[label], :] = self.predict_over_c(
                        X=X[mask_y[label], :],
                        i=i,
                        j=j,
                        k=None,
                    ...
                    for k in range(self.c):
                        out[mask_y[label], k] = self.predict_over_c(
                            X=X[mask_y[label], :],
                            i=i,
                            j=j,
                            k=k,
    ...
    def my_model_imputation(
        self, t, y, X_miss, sde=None, mask_y=None, dmat=False, X_covs=None
    ):
        ...
        if self.diffusion_type == "vp":
            ...
            X = (
                t * X_miss + (1 - t) * y0
            )  # interpolation based on ground-truth for non-missing data
    ...
    def generate(self, batch_size=None, n_t=None, X_covs=None):
        ...
        y0 = np.random.normal(
            size=(self.b if batch_size is None else batch_size, self.c)
        )

        # Generate random labels
        label_y = self.y_uniques[
            np.argmax(np.random.multinomial(1, self.y_probs, size=y0.shape[0]), axis=1)
        ]
        ...
        my_model = partial(
            self.my_model, mask_y=mask_y, dmat=self.n_batch > 0, X_covs=X_covs
        )
        ...
        if self.diffusion_type == "vp":
            ...
            ode_solved = get_pc_sampler(my_model, sde=sde, denoise=True, eps=self.eps)(
                y0.reshape(-1)
            )
        else:
            ode_solved = euler_solve(
                my_model=my_model, y0=y0.reshape(-1), N=self.n_t if n_t is None else n_t
            )  # [t, b*c]
        ...
        if self.label_y is not None:
            solution = np.concatenate(
                (solution, np.expand_dims(label_y, axis=1)), axis=1
            )
    ...
    def impute(
        ...
        my_model_imputation = partial(
            self.my_model_imputation,
            X_miss=X,
            sde=sde,
            mask_y=mask_y,
            dmat=self.n_batch > 0,
        ...
        for i in range(k):
            ...
            solution = copy.deepcopy(
                X
            )  # Solution start with dataset which contains some missing values
            if self.diffusion_type == "vp":
                ode_solved = get_pc_sampler(
                    my_model_imputation, sde=sde, denoise=True, repaint=repaint
                )(y0_miss, r=r, j=int(math.ceil(j * n_t)))
                solution[mask_miss] = (
                    ode_solved  # replace missing values with imputed values
                )
            solution = self.unscale(solution)
            solution = self.clean_onehot_data(solution)
            solution = self.clip_extremes(solution)
            ...
            if i == 0:
                ...
                imputed_data = np.concatenate(
                    (imputed_data, np.expand_dims(solution, axis=0)), axis=0
                )
    ...
    def zero_shot_classification(self, x, n_t=10, n_z=10, X_covs=None):
        ...
        for i in range(num_classes):  # for each class
            ...
            for k in range(n_z):  # monte-carlo over multiple noises
                ...
                for j in range(
                    n_t - 1
                ):  # averaging over multiple noise levels [t=1/n, ... (n-1)/n]
                    t = t + h
                    ...
                    if self.diffusion_type == "flow":
                        ...
                        x0 = pred_  # x0
                    L2_dist_ += [
                        np.expand_dims(np.sum((x0 - y0) ** 2, axis=1), axis=0)
                    ]  # [1, b]
    ...
    def predict(self, X, n_t=None, n_z=None, X_covs=None):
        ...
        most_likely_class_avg, prob_avg = self.zero_shot_classification(
            X, n_t=n_t, n_z=n_z, X_covs=X_covs
        )
