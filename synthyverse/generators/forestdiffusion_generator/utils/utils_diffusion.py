...
class IterForDMatrix(xgb.core.DataIter):
    ...
    def __init__(
        self, data, t, seed, n_batch=1, n_epochs=100, diffusion_type="flow", sde=None
    ):
        self._data = data
        self.seed = seed
        ...
        self.batch_slices = [
            slice(i * rows_per_batch, (i + 1) * rows_per_batch)
    ...
    def next(self, input_data):
        ...
        if self.it == self.n_batch * self.n_epochs:  # stops after k epochs
            return 0
        x_t, z = get_xt(
            x0=self._data[self.batch_slices[self.it % self.n_batch]],
            t=self.t,
            rng=self.rng,
...
def build_data_iterator(x0, t, seed, n_batch, duplicate_K, diffusion_type, sde):
    return IterForDMatrix(
        x0,
        t,
        seed,
        n_batch=n_batch,
        n_epochs=duplicate_K,
