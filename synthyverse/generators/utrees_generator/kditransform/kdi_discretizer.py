...
from .kdi_transformer import KDITransformer
...
class KDDiscretizer(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X):
        ...
        for cix in range(n_features):
            ...
            if n_unique_preds != self.n_bins_[cix]:
                warnings.warn(
                    "{}: n_unique_preds={}, n_clusters={}".format(
    ...
    def get_intervals(self, idx):
        ...
        """
        intervals = np.c_[
            np.r_[np.NINF, self.boundaries_[idx]], np.r_[self.boundaries_[idx], np.PINF]
        ]
        return intervals
    ...
    def predict(self, X):
        ...
        pred[np.arange(pred.shape[0]), np.argmax(proba, axis=1)] = 1.0
        return pred
    ...
    def predict_proba(self, X):
        ...
        lnorm = pis + np.log(
            np.sum(np.exp(unnorm_logprobas - pis), axis=1, keepdims=True)
        )
...
class KBinsKDIDiscretizer(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X):
        n_samples, n_features = X.shape
        self.n_features_in_ = n_features
        scott = np.power(n_samples * (1 + 2.0) / 4.0, -1.0 / (1 + 4))
